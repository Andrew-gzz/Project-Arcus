...
export const getSalesByCategory = async (req, res) => {
  try {
    ...
    logger.error(`Error en getSalesByCategory: ${error.message}`, { stack: error.stack });
...
export const getTopProducts = async (req, res) => {
  try {
    ...
    logger.error(`Error en getTopProducts: ${error.message}`, { stack: error.stack });
    res.status(500).json({ message: error.message });
  }
};
...
export const getTopUsers = async (req, res) => {
  try {
    ...
    logger.error(`Error en getTopUsers: ${error.message}`, { stack: error.stack });
    res.status(500).json({ message: error.message });
  }
};
...
export const getSubscriptionStats = async (req, res) => {
  try {
    ...
    logger.error(`Error en getSubscriptionStats: ${error.message}`, { stack: error.stack });
    res.status(500).json({ message: error.message });
  }
};
